...
export function GamePage({ onGoToCollection, onGameStateChange }: { onGoToCollection: () => void; onGameStateChange?: (active: boolean) => void }) {
  ...
  const finish = useCallback(() => { setPhase("finished"); setFalling([]); direction.current.clear(); }, []);
  const start = () => {
  ...
  useEffect(() => {
    ...
    const frame = (now: number) => {
      ...
      setFalling((current) => { let gained = 0; let caughtNow = 0; const next = current.flatMap((fallingItem) => { const y = fallingItem.y + delta * .00038 * height; const hit = y + fallingItem.size >= height - 48 && y <= height - 18 && Math.abs(fallingItem.x - currentBasketX) < (basketWidth + fallingItem.size) / 2; if (hit) { gained += CATCH_SCORE; caughtNow += 1; return []; } return y > height + fallingItem.size ? [] : [{ ...fallingItem, y }]; }); if (gained) { setScore((value) => value + gained); setCaught((value) => value + caughtNow); } return next; });
      if (Math.floor((now - startedAt.current) / 900) > sequence.current) { sequence.current += 1; const item = snapshot.current[pickRandomIndex(Math.random(), snapshot.current.length) ?? 0]; if (item) setFalling((current) => [...current, { id: `${item.id}-${sequence.current}`, x: randomSpawnX(Math.random(), width, 54), y: -60, size: 54, item }]); }
  ...
  useEffect(() => {
    ...
    const down = (event: KeyboardEvent) => { const key = keyMap[event.key]; if (!key || (phase !== "playing" && phase !== "paused")) return; event.preventDefault(); direction.current.press(event.key, key, event.repeat); };
    const up = (event: KeyboardEvent) => { if (keyMap[event.key]) direction.current.release(event.key); };
    const pause = () => { if (phase === "playing") { direction.current.clear(); setPhase("paused"); } };
